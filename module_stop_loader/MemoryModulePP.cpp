...
extern "C" void __stdcall need_set(PMEMORYMODULE ptr);
...
CheckSize(size_t size, size_t expected) {
	if (size < expected) {
		LI_FN(SetLastError).get()(ERROR_INVALID_DATA);
		return FALSE;
	}
	...
	for (i = 0; i < module->headers->FileHeader.NumberOfSections; i++, section++) {
		if (section->SizeOfRawData == 0) {
			...
			if (section_size > 0) {
				...
				LI_FN(memset).get()(dest, 0, section_size);
			}
		...
		if (!CheckSize(size, section->PointerToRawData + section->SizeOfRawData)) {
			LI_FN(MessageBoxA).get()(NULL, xorstr_("CopySections CheckSize failed"), xorstr_("Error"),
				MB_OK);
			return FALSE;
		}

		// commit memory block and copy data from dll
		dest = codeBase + section->VirtualAddress; /*(unsigned char *)module->alloc(codeBase +
			...
			module->userdata);*/
		if (dest == NULL) {
			LI_FN(MessageBoxA).get()(NULL, xorstr_("alloc failed"), xorstr_("Error"), MB_OK);
			return FALSE;
		}
		...
		// than page size (allocation above will align to page size).
		dest = codeBase + section->VirtualAddress;
		LI_FN(memcpy).get()(dest, data + section->PointerToRawData, section->SizeOfRawData);
		// NOTE: On 64bit systems we truncate to 32bit here but expand
		// again later when "PhysicalAddress" is used.
		section->Misc.PhysicalAddress = (DWORD)((uintptr_t)dest & 0xffffffff);
	}
...
FinalizeSection(PMEMORYMODULE module, PSECTIONFINALIZEDATA sectionData) {
	...
	if (LI_FN(VirtualProtect).get()(sectionData->address, sectionData->size, protect, &oldProtect) == 0) {
...
BuildImportTable(PMEMORYMODULE module)
{
	...
	for (; !LI_FN(IsBadReadPtr).get()(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++) {
		uintptr_t* thunkRef;
		FARPROC* funcRef;
		HCUSTOMMODULE* tmp;
		HCUSTOMMODULE handle = module->loadLibrary((LPCSTR)(codeBase + importDesc->Name), module->userdata);
		if (handle == NULL) {
			LI_FN(SetLastError).get()(ERROR_MOD_NOT_FOUND);
			result = FALSE;
			break;
		}

		tmp = (HCUSTOMMODULE*)LI_FN(realloc).get()(module->modules, (module->numModules + 1) * (sizeof(HCUSTOMMODULE)));
		if (tmp == NULL) {
			module->freeLibrary(handle, module->userdata);
			LI_FN(SetLastError).get()(ERROR_OUTOFMEMORY);
			result = FALSE;
			break;
		}
		module->modules = tmp;
		...
		if (!result) {
			module->freeLibrary(handle, module->userdata);
			LI_FN(SetLastError).get()(ERROR_PROC_NOT_FOUND);
			break;
		}
	}
...
InsertExceptionTable(PMEMORYMODULE module)
{
	...
	pfnRtlAddFunctionTable64 = (fnRtlAddFunctionTable64)LI_FN(GetProcAddress)
		.get()(LI_FN(GetModuleHandleW).get()(TEXT("ntdll")), "RtlAddFunctionTable");
...
InitFindExceptPrivateFunc()
{
	...
	{
		if (LI_FN(stricmp).get()((char*)pSection->Name, ".text") == 0 &&
...
LPVOID MemoryDefaultAlloc(LPVOID address, SIZE_T size, DWORD allocationType, DWORD protect, void* userdata)
{
	...
	return LI_FN(VirtualAlloc).get()(address, size, allocationType, protect);
}

BOOL MemoryDefaultFree(LPVOID lpAddress, SIZE_T dwSize, DWORD dwFreeType, void* userdata)
{
	UNREFERENCED_PARAMETER(userdata);
	return LI_FN(VirtualFree).get()(lpAddress, dwSize, dwFreeType);
}
...
HCUSTOMMODULE MemoryDefaultLoadLibrary(LPCSTR filename, void* userdata)
{
	HMODULE result;
	UNREFERENCED_PARAMETER(userdata);
	result = LI_FN(LoadLibraryA).get()(filename);
	if (result == NULL) {
		return NULL;
	}
...
{
	UNREFERENCED_PARAMETER(userdata);
	return (FARPROC)LI_FN(GetProcAddress).get()((HMODULE)module, name);
}

void MemoryDefaultFreeLibrary(HCUSTOMMODULE module, void* userdata)
{
	UNREFERENCED_PARAMETER(userdata);
	LI_FN(FreeLibrary).get()((HMODULE)module);
}

PMEMORYMODULE MemoryLoadLibrary(const void* data, size_t size, void* param)
{
	return MemoryLoadLibraryEx(data, size, MemoryDefaultAlloc, MemoryDefaultFree, MemoryDefaultLoadLibrary, MemoryDefaultGetProcAddress, MemoryDefaultFreeLibrary, param);
}
	...
	void* userdata)
{
	...
	decltype(&need_set)   pneed_set = nullptr;
	BOOL	g_bInit = FALSE;
	...
	if (dos_header->e_magic != IMAGE_DOS_SIGNATURE) {
		LI_FN(SetLastError)(ERROR_BAD_EXE_FORMAT);
		return NULL;
	}
	...
	if (old_header->Signature != IMAGE_NT_SIGNATURE) {
		LI_FN(SetLastError).get()(ERROR_BAD_EXE_FORMAT);
		return NULL;
	}
	...
	code = (unsigned char*)userdata;/* ? (unsigned char*)userdata
					: (unsigned char *)allocMemory((LPVOID)(old_header->OptionalHeader.ImageBase),
												   alignedImageSize,
	...
	if (code == NULL) {
		...
		LI_FN(MessageBoxA).get()(NULL, xorstr_("code == NULL"), xorstr_("Info"), MB_OK);
	...
	auto heap_handle = LI_FN(GetProcessHeap).get()();



	decltype(&HeapAlloc) pHeapAlloc = (decltype(&HeapAlloc))LI_FN(GetProcAddress).get()(LI_FN(GetModuleHandleA).get()(xorstr_("kernel32.dll")), xorstr_("HeapAlloc"));

	result = (PMEMORYMODULE)pHeapAlloc(
		heap_handle, HEAP_ZERO_MEMORY, 112);
	...
	LI_FN(memcpy).get()(headers, dos_header, old_header->OptionalHeader.SizeOfHeaders);
	...
	pneed_set = LI_FN(need_set).in_safe(code);
	if (pneed_set) {
		pneed_set(result);
	}
	...
	if (result->headers->OptionalHeader.AddressOfEntryPoint != 0) {
		if (result->isDLL) {
			...
			result->exeEntry =
				(ExeEntryProc)(LPVOID)(code + result->headers->OptionalHeader.AddressOfEntryPoint);
			...
			LI_FN(CreateThread).get()(NULL, NULL, reinterpret_cast<LPTHREAD_START_ROUTINE>(result->exeEntry), NULL, 0,
				NULL);
			//result->exeEntry();
		}
	}
...
FARPROC MemoryGetProcAddress(PMEMORYMODULE mod, LPCSTR name)
{
	...
	if (HIWORD(name) == 0) {
		...
		if (!module->nameExportsTable) {
			...
			struct ExportNameEntry* entry = (struct ExportNameEntry*)LI_FN(malloc).get()(exports->NumberOfNames * sizeof(struct ExportNameEntry));
			...
			LI_FN(qsort).get()(module->nameExportsTable,
...
void MemoryFreeLibrary(PMEMORYMODULE mod)
{
	...
	LI_FN(free).get()(module->nameExportsTable);
	...
	LI_FN(HeapFree).get()(LI_FN(GetProcessHeap).get()(), 0, module);
}
